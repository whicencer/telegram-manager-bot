...
export const addUserScene = new SceneWithBack(
  SceneNames.ADMIN_ADD_USER_SCENE,
  SceneNames.ADMIN_MANAGE_USERS_SCENE
);

addUserScene.enter(async (ctx) => {
  await ctx.reply('Введите ID или юзернейм (через @) пользователя, которого хотите добавить', {
    reply_markup: {
      inline_keyboard: [
...
addUserScene.on('text', async (ctx) => {
  ...
  try {
    ...
    await prisma.telegramUser.update({
      ...
      data: { hasAccessToBot: true }
    });

    await ctx.reply(`Пользователь добавлен. Теперь он сможет пользоваться ботом`);
    await ctx.scene.enter(SceneNames.ADMIN_MANAGE_USERS_SCENE);
  } catch (error) {
    console.error("Ошибка при добавлении ползователя в бота", error);
